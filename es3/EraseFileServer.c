	...
	*/
	...
	for(;;){ 
		...
		if (fork()==0){ // figlio
            ...
            if ( read(conn_sd,&line,sizeof(line)) <= 0 ) {
                perror("reading line to erase");
                continue;
            }

            line = ntohl(line);
            
            printf("\nLinea da cancellare : %i\n", line);
            
            line = line - 1;

            while((nread=read(conn_sd, &buf, 1)) > 0){
				printf("\n%i - %i",buf,nread);
                if ( currentLine != line ) {
                    write(conn_sd,&buf,nread);
                }
